...
def test_config_equality_ignores_root(tmp_path):
    cfg_file = tmp_path / "routing.yaml"
    cfg_file.write_text(
        "data_root: ./data\n"
        "index_db: ./data/index.db\n"
    ...
    b = load_config(cfg_file, root=tmp_path)
    assert a == b
    r = repr(a)
    assert "data_root=" in r  # sanity: fields still present
    # The private _root field must be suppressed (repr=False).
    # Use a split on "data_root" to avoid false-matching it.
    assert r.count(", _root=") == 0 and not r.startswith("_root=")
